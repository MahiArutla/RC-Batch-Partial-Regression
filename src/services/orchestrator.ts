...
export class Orchestrator {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // BNS COMM NF Happy Path
  // ─────────────────────────────────────────────────────────────────────────────
  async runBnsCommNfHappyPath(page: Page, scenarioId: string): Promise<FileDetails> {
    const fileDetails = loadScenarioData(scenarioId);
    ...
    await hangfirePage.goToProcessHFJobs(db, fileDetails);
    await db.validateHandshakeJobStatus(fileDetails);
    console.log('Handshake job status validated in DB for BNS COMM NF');
    ...
    console.log(
      `BNS COMM NF file processed with Batchnumber ${fileDetails.batchNumber}, ` +
      `filename ${fileDetails.inputFileName}, PartnerReference ${fileDetails.partnerReference}, ` +
      `RegistrationNumber ${fileDetails.baseRegistrationNum}, OrderId ${fileDetails.orderId}, ` +
      `and ReturnFile ${fileDetails.returnFileName}`
    );
  ...
  async runBnsCommAmendmentHappyPath(
    ...
    const fileDetails = loadScenarioData(scenarioId);
    fileDetails.sampleFile = path.resolve(process.cwd(), 'src', 'data', 'BNS_COMM', 'BNS_Comm_Amend.xml');
    fileDetails.scenarioId = scenarioId;
    fileDetails.partnerReference = partnerReference;
    fileDetails.baseRegistrationNum = registrationNumber;
    ...
    await fileSystem.createBnsCommAmendmentXml(fileDetails);

    const db = new DbService();
    fileDetails.batchType = 'Amendment';
    await db.setProcessAndFileStatusToNotStarted(fileDetails);
    const hangfirePage = new HangfireJobsPage(page);
    await hangfirePage.goToProcessHFJobs(db, fileDetails);
    await db.validateHandshakeJobStatus(fileDetails);
    console.log('Handshake job status validated in DB for BNS COMM Amendment');
    ...
    console.log(
      `BNS COMM Amendment file processed with Batchnumber ${fileDetails.batchNumber}, ` +
      `filename ${fileDetails.inputFileName}, PartnerReference ${fileDetails.partnerReference}, ` +
      `RegistrationNumber ${fileDetails.baseRegistrationNum}, OrderId ${fileDetails.orderId}, ` +
      `and ReturnFile ${fileDetails.returnFileName}`
    );
